...
while i==114081:
    ...
    stats_veg=grass.read_command('r.stats',input='vegcompltemp')
    grass.run_command('g.region',rast='vegcompltemp')
    stats_veg_split=stats_veg.split('\n')
    del stats_veg_split[-1]
    del stats_veg_split[-1]
    listmapa_contruir=[]
    #grass.mapcalc('mapa_reconstruido=0', overwrite = True, quiet = True)
    print len(stats_veg_split)
    for x in stats_veg_split:
        print x
